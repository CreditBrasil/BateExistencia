unit BE.Configuracao;

interface

uses
  System.SysUtils, System.IniFiles, MVC.Iniciadores;
...
const
  NomeBancoDeDados: array [TBancoDeDados] of string = ('MS SQL Server', //bdMsSQLServer
    'Oracle' //bdOracle
    );
...
begin
  ...
  try
    FADOConnectionString := LIniFile.ReadString('Conexao', 'ADOConnectionString',
      'Falta definir o ADOConnectionString no GlobalFinanceiro.ini');
...
begin
  Configuracao := TConfiguracao.Create(ExtractFilePath(GetModuleName(HInstance)) + 'BateExistencia.ini');
end;
