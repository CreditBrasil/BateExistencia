...
uses
  System.SysUtils, System.Variants, System.SyncObjs, Winapi.ADOInt, Core.ServiceLocator, AR.ActiveRecord,
  MVC.Iniciadores, BE.Configuracao {, uCtrl_IdModel};

function CriaConexao_BE: IActiveRecordConnection;
...
type
  ...
  TActiveRecordDependenciaProjetoMsSQLServer = class(TActiveRecordDependenciaProjetoGenerico,
  ...
  TActiveRecordDependenciaProjetoOracle = class(TActiveRecordDependenciaProjetoGenerico,
    IActiveRecordDependenciaProjeto)
  protected
    function SincronizaDatabaseNow(const AConnection: IActiveRecordConnection): TDateTime; override;
    { IActiveRecordDependenciaProjeto }
...
function TActiveRecordDependenciaProjetoMsSQLServer.SincronizaDatabaseNow(const AConnection: IActiveRecordConnection)
  : TDateTime;
var
...
function TActiveRecordDependenciaProjetoOracle.SincronizaDatabaseNow(const AConnection: IActiveRecordConnection)
  : TDateTime;
var
  LRecordset: _Recordset;
  LResultado: string;
  ...
  LRegistrosAfetados: OleVariant;
begin
  LRecordset := (AConnection as IActiveRecordConnectionADO)
    .ADOConnection.Execute('SELECT TO_char(' + SQLNow + ',''DD/MM/YYYY HH24:MI:SS'') FROM DUAL', LRegistrosAfetados, 0);
...
begin
  case Configuracao.BancoDeDados of
    ...
    bdOracle:
      ActiveRecordDependenciaProjeto := TActiveRecordDependenciaProjetoOracle.Create;
  end;
end;
