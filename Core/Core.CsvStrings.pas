...
type
  ...
  public
    constructor Create;
    constructor CreateParams(ASeparador, ALimitador: Char; AForcaLimitador: Boolean);
...
constructor TCSVStringList.CreateParams(ASeparador, ALimitador: Char; AForcaLimitador: Boolean);
begin
  Create;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        P := PChar(S);
        while not CharInSet(P^, [#0 .. ' ', FLimitador, FSeparador]) do
          P := CharNext(P);
        if (P^ <> #0) then
          S := AnsiQuotedStr(S, FLimitador);
      end;
...
begin
  ...
  begin
    while (Length(aux) > 0) and CharInSet(aux[1], [#1 .. ' ']) do
