...
uses
  System.SysUtils, System.Classes, System.Contnrs, System.Variants, Core.Tipos;
...
type
  ...
  public
    ...
    function StringCodigos(const Delimitador: string = ''; const Separador: string = ','): string; overload;
    function StringCodigos(const Delimitador, Separador, UltimoSeparador: string): string; overload;
  ...
  public
    ...
    property Value[ACodigo: Integer]: Double read GetValue write SetValue; default;
  end;
  ...
  public
    ...
    property FormatoSubstituicao: string read FFormatoSubstituicao write FFormatoSubstituicao;
    property Valor1: Double read GetValor1 write SetValor1;
    property Valor2: Double read GetValor2 write SetValor2;
  end;
  ...
  public
    ...
    property Value[const ACodigo: string]: string read GetValue write SetValue; default;
  end;
  ...
  public
    ...
    property Objects[ACodigo: TClass]: TObject read GetObjects write SetObjects; default;
...
begin
  if not Origem.InheritsFrom(ClassType) then
    raise Exception.CreateFmt('%s não é descendente de %s', [Origem.ClassName, ClassName]);
  if not Origem.FClasse.InheritsFrom(FClasse) then
    raise Exception.CreateFmt('%s não é descendente de %s', [Origem.FClasse.ClassName, FClasse.ClassName]);
  Clear;
...
begin
  ...
  begin
    ...
    begin
      if (FGuardaPosicao < FLista.Count) and (FLista[FGuardaPosicao] = FGuardaObjeto) then
...
procedure TCache.SetObjetoPosicao(AObjeto: TItemCache; const Posicao: Integer);
begin
  FObjeto := AObjeto;
  FPosicaoAtual := Posicao;
end;

function TCache.StringCodigos(const Delimitador: string; const Separador: string): string;
var
  laco: Integer;
begin
  Result := '';
...
function TCache.StringCodigos(const Delimitador, Separador, UltimoSeparador: string): string;
var
  laco: Integer;
...
begin
  ...
  try
    ...
    begin
      Posicao := laco;
      Result := Result + Delimitador + StringCodigo + Delimitador + LUltimoSeparador
        [(Count >= 2) and (laco = Count - 2)];
    end;
    Delete(Result, Length(Result) - Length(Separador) + 1, Length(Separador));
  finally
    EndUpdate;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Result := SubstitueTexto(Copy(Ent, 1, P - 1) + aux + Copy(Ent, P + F + Length(DelimE) + Length(DelimD) - 1,
          Length(Ent)), DelimE, DelimD);
      end;
    end;
  finally
    EndUpdate;
...
procedure THashTable.SetAsBoolean(const ACodigo: string; const AValue: Boolean);
begin
...
begin
  ...
  Result := Ord(Integer(LCodigo) > Integer(FNovoCodigo)) - Ord(Integer(LCodigo) < Integer(FNovoCodigo));
end;
...
procedure THashClassObject.SetObjects(ACodigo: TClass; const Value: TObject);
begin
  Codigo := ACodigo;
  Object_ := Value;
end;
