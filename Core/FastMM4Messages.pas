...
const
  ...
  VirtualMethodErrorHeader =
    'FastMM detectou uma tentativa de chamada a um método virtual de um objeto liberado. Uma violação de acesso será disparada para abortar a operação corrente.';
  InterfaceErrorHeader =
    'FastMM detectou uma tentativa de uso de uma interface de um objeto liberado. Uma violação de acesso será disparada para abortar a operação corrente.';
  BlockHeaderCorruptedNoHistoryMsg =
    ' Infelizmente o cabeçalho do bloco foi corrompido e a história não está disponível.';
  ...
  OtherMMInstalledMsg = 'FastMM4 não pode ser instalado já que outro gerenciador externo ' +
    ...
    'tenha certeza que a unit FastMM4.pas seja a primeira na seção "uses"' + #13#10'do arquivo .dpr do seu projeto.';
