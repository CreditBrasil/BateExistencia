...
uses
  System.SysUtils, Core.Metodos, Core.ServiceLocator {, uBancosModel};
...
begin
  ...
  Result := (Length(Trim(ACMC7)) = 34) and (ACMC7[1] = '<') and (ACMC7[10] = '<') and (ACMC7[21] = '>') and
    (ACMC7[34] = ':');
end;
...
begin
  ...
  C1 := DigitoCMC7(A1);
  A2 := TMetodos.PoeLetraEsq(TMetodos.SoNumero(APraca), 3, '0') + TMetodos.PoeLetraEsq(TMetodos.SoNumero(ACheque), 6,
    '0') + '5';
  C2 := DigitoCMC7(A2);
...
class function TCMC7Metodos.ContaAjustadoTamanho(const ACMC7: string; ATamanhoConta: Integer): string;
begin
