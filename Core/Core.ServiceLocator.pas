...
{$ENDIF}
  ...
  IServiceLocator = interface
    ...
    procedure AddSLClass(const IID: TGUID; ASLClass: TSLClass);
    procedure AddFunctionFactory(const IID: TGUID; AFunctionFactory: TFunctionFactory);
    procedure AddInstance(const IID: TGUID; const AInstance: IUnknown);
{$IFDEF Test}
    procedure AddSLClassForTest(const IID: TGUID; ASLClass: TSLClass);
    procedure AddFunctionFactoryForTest(const IID: TGUID; AFunctionFactory: TFunctionFactory);
    procedure AddInstanceForTest(const IID: TGUID; const AInstance: IUnknown);
{$ENDIF}
...
procedure TServiceLocator.AddSLClass(const IID: TGUID; ASLClass: TSLClass);
begin
  Assert(not Exist(IID));
  FFactories[IID] := TFactoryBySLClassCreate.Create(IID, ASLClass);
end;

procedure TServiceLocator.AddFunctionFactory(const IID: TGUID; AFunctionFactory: TFunctionFactory);
begin
  Assert(not Exist(IID));
  FFactories[IID] := TFactoryByFunctionFactory.Create(IID, AFunctionFactory);
end;

procedure TServiceLocator.AddInstance(const IID: TGUID; const AInstance: IUnknown);
begin
  Assert(not Exist(IID));
...
procedure TServiceLocator.AddSLClassForTest(const IID: TGUID; ASLClass: TSLClass);
begin
  Assert(Exist(IID));
...
procedure TServiceLocator.AddFunctionFactoryForTest(const IID: TGUID; AFunctionFactory: TFunctionFactory);
begin
  Assert(Exist(IID));
  Assert(not FFactoriesTest.Exist(IID));
...
procedure TServiceLocator.AddInstanceForTest(const IID: TGUID; const AInstance: IUnknown);
begin
  Assert(Exist(IID));
  Assert(not FFactoriesTest.Exist(IID));
