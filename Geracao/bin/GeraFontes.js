...
function geraModel(arquivo) {
  ...
  propModel.NewProperty("NomeModel", Tabela.Nome, typeString);
  if (propModel.Show("Geração de Model")) {
    nomeModel = propModel("NomeModel");
    saida = "";
    var debug = compila(leArquivo("Templates\\Model.pas_template"));
    if (GravaArquivoDebug)
      gravaArquivo("Debug\\Modelo." + nomeModel + "_pas.js", debug);
    eval(debug);
    gravaArquivo(pastaOutput + "\\Modelo\\Modelo." + nomeModel + ".pas", saida);
  }
}
...
function geraWEBControlador(arquivo) {
  var Tabela = getTabela(arquivo);
  var propModel = WScript.CreateObject("Helper.Properties");
  propModel.NewProperty("NomeProjeto", "GF", typeString);
  propModel.NewProperty("NomeModel", Tabela.Nome, typeString);
  ...
  if (propModel.Show("Geração do Web Controlador")) {
    ...
    if (GravaArquivoDebug)
      gravaArquivo("Debug\\" + nomeProjeto + ".Controlador." + nomeControlador + ".pas.js", debug);
    eval(debug);
    gravaArquivo(pastaOutputDelphi7 + "Controladores\\" + nomeProjeto + ".Controlador." + nomeControlador + ".pas", saida);
  }
}
...
function geraWEBPagina(arquivo) {
  ...
  if (propModel.Show("Geração da Web Página")) {
    ...
    if (GravaArquivoDebug)
      gravaArquivo("Debug\\" + nomeProjeto + "." + nomeControlador + "." + nomeVisao + ".html.js", debug);
    eval(debug);
    gravaArquivo(pastaOutputDelphi7 + "Visoes\\HTML\\" + nomeProjeto + ".Visao." + nomeControlador + "." + nomeVisao + ".html", saida);
  }
}
