unit AR.ActiveRecord;

interface

uses
  System.SysUtils, System.Classes, System.Contnrs, System.Variants, System.Math,
  Vcl.Graphics, Data.DB, Winapi.ADOInt,
  Core.Singleton, Core.ServiceLocator;
...
type
  ...
  IActiveRecordQueryFactory = interface
    ...
    function CreateQuery(const AConnection: IActiveRecordConnection; AClassActiveRecord: TClassActiveRecord = nil)
      : IActiveRecordQuery;
  ...
  public
    ...
    class function ToString_(const AValue: TNullableColor): string; overload;
    class function ToString_(const AValue: TNullableCurrency): string; overload;
    class function ToString_(const AValue: TNullableDateTime): string; overload;
    class function ToString_(const AValue: TNullableExtended): string; overload;
    class function ToString_(const AValue: TNullableInteger): string; overload;
    class function ToString_(const AValue: TNullableString): string; overload;
    class function ToString_(const AValue: TNullableWideString): string; overload;
    ...
    class function ValidateString(const AValue: string; const AFieldMetadata: TFieldMetadata;
  ...
  IActiveRecordTotalizacao = interface
    ...
    function MediaPonderada(AFieldIndex, AFieldIndexDePonderacao: Integer; const AFormatDisplayText: string = ',0.00')
  ...
  protected
    ...
    function CreateQuery(const AConnection: IActiveRecordConnection; AClassActiveRecord: TClassActiveRecord = nil)
      : IActiveRecordQuery; virtual; abstract;
    function OrderBy(const AClause: string): IActiveRecordQueryFactory;
    function Top(ACount: Integer): IActiveRecordQueryFactory;
    function Where(const AClause: string): IActiveRecordQueryFactory;
  public
...
const
  ...
  ColumnCharWidthForMax = 30;
  EquivalenciaFieldTypeEVariant: array [TFieldTypeMetadata] of Integer = (varDate, //arftDateTime
    varDouble, //arftFloat
...
constructor EActiveRecordConstraintField.CreateMetadata(const Msg: string; ATableMetadata: TTableMetadata;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if LFieldMetadata.Visible and (not LFieldMetadata.Commented) and
          (AnsiPos(LText, AnsiUpperCase(GetAccess(laco).DisplayText(LFieldMetadata.Index))) > 0) then
        begin
          LEncontrou := True;
          Break;
        end;
...
class function TIteratorCollectionActiveRecordDecorator.Decore(const ACollectionActiveRecord: ICollectionActiveRecord)
  : IIteratorActiveRecord;
begin
...
class function TIteratorActiveRecordQueryDecorator.Decore(const AActiveRecordQuery: IActiveRecordQuery)
  : IIteratorActiveRecord;
begin
...
begin
  raise EActiveRecord.CreateFmt
    ('Não existe acesso ou o campo não tem valor padrão ou é somente leitura para o índice %d.', [Index]);
end;
...
begin
  Result := AValue1.Null and AValue2.Null or ((not AValue1.Null) and (not AValue2.Null) and
...
class function TActiveRecord.ToString_(const AValue: TNullableColor): string;
begin
  if AValue.Null then
    Result := ''
  else
...
class function TActiveRecord.ToString_(const AValue: TNullableCurrency): string;
begin
  if AValue.Null then
    Result := ''
  else
...
class function TActiveRecord.ToString_(const AValue: TNullableDateTime): string;
begin
  if AValue.Null then
    Result := ''
  else
...
class function TActiveRecord.ToString_(const AValue: TNullableExtended): string;
begin
  if AValue.Null then
    Result := ''
  else
...
class function TActiveRecord.ToString_(const AValue: TNullableInteger): string;
begin
  if AValue.Null then
    Result := ''
  else
...
class function TActiveRecord.ToString_(const AValue: TNullableString): string;
begin
  if AValue.Null then
    Result := ''
  else
...
class function TActiveRecord.ToString_(const AValue: TNullableWideString): string;
begin
  if AValue.Null then
    Result := ''
  else
...
class function TActiveRecord.ValidateString(const AValue: string; const AFieldMetadata: TFieldMetadata;
  ...
  begin
    if not TryStrToDateTime(AValue, LValue) then
      Result := '"' + AValue + '" não corresponde a uma data válida. Deve ser no formato ' +
...
class function TResultadoSQL.Execute(const ACommandSQL: string; const AConnection: IActiveRecordConnection)
  : IActiveRecordQuery;
begin
...
begin
  ...
  ConfigFieldMetadata(AIndexTo).SetName(AFieldMetadata.Name).SetLength(AFieldMetadata.Length)
    .SetNullable(AFieldMetadata.Nullable).SetCaption(AFieldMetadata.Caption).SetVisible(AFieldMetadata.Visible)
  //.SetReadOnly(AFieldMetadata.ReadOnly)
  //.SetCommented(AFieldMetadata.Commented)
    .SetTitle(AFieldMetadata.Title).SetColumnCharWidth(AFieldMetadata.ColumnCharWidth).Write;
end;
...
begin
  ...
  //.SetName(AFieldMetadata.Name)
    .SetLength(AFieldMetadata.Length).SetNullable(AFieldMetadata.Nullable).SetCaption(AFieldMetadata.Caption)
...
function TActiveRecordMetadataWritable.FromMetadataDo(const AActiveRecordMetadata: IActiveRecordMetadata)
  : IFromMetadataDo;
begin
...
function TActiveRecordTotalizacao.AdicionaTotalizador(const ATotalizador: IActiveRecordTotalizador)
  : IActiveRecordTotalizacao;
begin
  Result := Self;
...
function TActiveRecordTotalizacao.Somar(AFieldIndex: Integer; const AFormatDisplayText: string)
  : IActiveRecordTotalizacao;
begin
...
begin
  ...
  begin
    ...
    LLimitadorCondicionalQuantidadeRegistros := ActiveRecordDependenciaProjeto.LimitadorCondicionalQuantidadeRegistros
      (FTop, False);
  end;
...
var
  ...
  begin
    ...
    if LMetadata.TableMetadata.DeletedField <> '' then
      Result := 'a.' + LowerCase(LMetadata.TableMetadata.DeletedField) + ' is null';
...
begin
  ...
  try
    ...
    if LLimitadorSeletivoQuantidadeRegistros <> '' then
      Add('  ' + LLimitadorSeletivoQuantidadeRegistros);
    for laco := 0 to LMetadata.FieldCount - 2 do
      Add('  a.' + LowerCase(LMetadata.FieldMetadata(laco).Name) + ' a_' + LowerCase(LMetadata.FieldMetadata(laco)
        .Name) + ',');
    ...
    begin
      ...
      if LRelationshipMetadata.Metadata <> nil then
        ...
        LMetadataJoin := LRelationshipMetadata.ClassActiveRecord.Metadata;
      LAlias := Chr(Ord('b') + laco);
      Add('  left join ' + LowerCase(LMetadataJoin.TableMetadata.Name) + ' ' + LAlias + ' on (' +
        OnClause(LAlias) + ')');
    end;
...
procedure TResultadoSQLQueryADO.DescobreMetadata(const LActiveRecordMetadataWritable: IActiveRecordMetadataWritable;
  ...
  begin
    case ADataType of
      ...
      adSingle, adDouble:
        Result := arftFloat;
      adCurrency, adDecimal, adNumeric:
        Result := arftMoney;
      adBoolean:
        Result := arftB01;
  ...
  begin
    ...
    case AFieldType of
      arftDateTime:
        Result := ColumnCharWidthForDateTime;
      arftFloat:
        Result := ColumnCharWidthForFloat;
      arftInteger:
        Result := ColumnCharWidthForInteger;
      arftMoney:
        Result := ColumnCharWidthForMoney;
      arftNS:
        Result := ColumnCharWidthForNS;
      arftVarchar:
        Result := ASize;
      arftMemo:
        Result := ColumnCharWidthForMemo;
      arftB01:
        Result := ColumnCharWidthForB01;
      arftColor:
        Result := ColumnCharWidthForColor;
    else
...
begin
  ...
  begin
    ...
    LActiveRecordMetadataWritable.ConfigNewFieldMetadata.SetName(AADORecordSet.Fields[laco].Name)
      .SetFieldType(LFieldType).SetLength(AADORecordSet.Fields[laco].DefinedSize).SetNullable(True)
      .SetCaption(AADORecordSet.Fields[laco].Name).SetVisible(True).SetReadOnly(True).SetCommented(False)
      .SetTitle(AADORecordSet.Fields[laco].Name)
      .SetColumnCharWidth(GetColumnCharWidth(LFieldType, AADORecordSet.Fields[laco].DefinedSize)).Write;
  end;
...
begin
  ...
  begin
    ...
    while (not FRecordSet.Eof) and (TActiveRecord.CheckVariant(FRecordSet.Fields[FPrimaryKeyLastFieldIndex[0]].Value)
      = LValorPrimaryKey) do
    begin
      for laco := 0 to High(FRelationships) do
        if (FRelationships[laco].RelationshipType = arrtOneToMany) and
